...
def _safe_relpath(p: Path) -> str:
    """Return a repo-relative path string when possible (avoid machine-specific absolutes)."""
    try:
...
def main() -> None:
    ...
    capsule_path_str = _safe_relpath(json_out)
    ...
    for v in variants:
        ...
        if ledger_path is not None:
            matrix_entry = MatrixEntry(
                ...
                hardware=hw,
                notes=f"{note} capsule={capsule_path_str}",
    ...
    print(f"\nWrote capsule: {capsule_path_str}")
